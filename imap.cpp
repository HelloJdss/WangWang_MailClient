...
bool iMap::recvMsg() {
  ...
  if (sock->waitForReadyRead(3000)) { //等待最多3秒
    ...
    while (sock->canReadLine()) {
      if (first) {
        this->MailtextBytes +=
            sock->bytesAvailable(); //读取本次可以读取的字节数
        first = false;
      }
  ...
  if (this->recvMsg()) {
    ...
    if (OK1) {
      ...
      for (int i = exists; i >= 1;
           i--, emit readprogress(exists - i, exists)) { //发送进度条信号
...
QString &iMap::getMailBody(qint32 index) {
  this->sendMsg("A004 fetch " + QString::number(this->Mailnum - index, 10) +
                " BODY[text]\r\n");
  bool OK1 = false;
  qint32 texttotal = 0;
  ...
  while ((OK1 = this->recvMsg())) { //循环接收，因为可能会被拆成多个报文段
    ...
    if (this->Mailbodytext.toLocal8Bit().size() == texttotal)
      break; //完全接受数据则不再等待
  }
  qDebug() << Mailbodytext.toLocal8Bit().size() << texttotal;
