    ...
    ui(new Ui::ClientWindow)
{
    ...
    Login_dialog.setModal(true); //设置为模态对话框，禁止在登陆之前与主窗口进行交互
    Login_dialog.show();
    Login_dialog.move((QApplication::desktop()->width()-Login_dialog.width())/2,(QApplication::desktop()->height()-Login_dialog.height())/2);//调整窗口位到屏幕中央
    ...
    if(isLogin()){
        ...
        //绑定操作信号
        QObject::connect(ui->action_undo,SIGNAL(triggered()),ui->WriteMail,SLOT(undo()));
        QObject::connect(ui->action_redo,SIGNAL(triggered()),ui->WriteMail,SLOT(redo()));
        QObject::connect(ui->action_copy,SIGNAL(triggered()),ui->WriteMail,SLOT(copy()));
        QObject::connect(ui->action_paste,SIGNAL(triggered()),ui->WriteMail,SLOT(paste()));
        QObject::connect(ui->action_selectAll,SIGNAL(triggered()),ui->WriteMail,SLOT(selectAll()));
        QObject::connect(ui->action_cut,SIGNAL(triggered()),ui->WriteMail,SLOT(cut()));
        //从服务器加载报文
